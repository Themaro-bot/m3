...
let handler = async (m, { conn, command, args }) => {
  ...
  if (global.db.data.users[m.sender].bank >= xpperlimit * count) {
    ...
    global.db.data.users[m.sender].miko += count
    conn.reply(m.chat, `Sukses menarik sebesar ${count} miko 💹`, m)
  } else conn.reply(m.chat, `[❗] Uang dibank anda tidak mencukupi untuk ditarik sebesar ${count} miko 💹`, m)
}
