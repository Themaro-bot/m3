      ...
      if (global.db.data.users[m.sender].bibitpisang > 499) {
        if (global.db.data.users[m.sender].bibitjeruk > 499) {
          if (global.db.data.users[m.sender].bibitanggur > 499) {
            let pisangpoin = `${Math.floor(Math.random() * 2000)}`.trim()
            let anggurpoin = `${Math.floor(Math.random() * 2000)}`.trim()
            let manggapoin = `${Math.floor(Math.random() * 2000)}`.trim()
            let jerukpoin = `${Math.floor(Math.random() * 2000)}`.trim()
            let apelpoin = `${Math.floor(Math.random() * 2000)}`.trim()
            ...
            var hsl = `Kamu menggunakan 500 bibit pisang, anggur, jeruk, mangga, dan apel dan mendapatkan : \n\n🍎 ${apelpoin}\n🍌 ${pisangpoin}\n🍇 ${anggurpoin}\n🥭 ${manggapoin}\n🍊 ${jerukpoin}`
