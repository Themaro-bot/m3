...
❌ *LOSE* ❌
Kamu kehilangan ${count} MIKO(xp)`.trim()
...
🎉 *WIN* 🎉
Kamu mendapatkan ${count * 2} MIKO(xp)`.trim()
...
🔖*DRAW* 🔖
Kamu mendapatkan ${count * 1} Miko(xp)`.trim()
global.db.data.users[m.sender].exp += count * 1
                conn.sendFile(m.chat, imgr + 'DRAW', 'casino.mp3', caption, m)
            }
        } else {
            conn.reply(m.chat, `Miko(xp) kamu tidak mencukupi untuk Casino silahkan *.claim* terlebih dahulu!`.trim(), m)
        }
    } catch (e) {
        console.log(e)
