...
let handler = async (m, { conn, text, participants, groupMetadata, usedPrefix }) => {
	if(isNaN(text)) {
  ...
  try {
    ...
    if(global.db.data.users[user].pasangan != m.sender){
      ...
      conn.reply(m.chat,`Kamu sudah berpacaran dengan @${user.split('@')[0]}, jika ingin putus silahkan ketik ${usedPrefix}putus `,m,{contextInfo: {
        mentionedJid: [user]
      }})
    }
    else{
