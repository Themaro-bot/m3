...
let handler = async (m, {
  ...
  try {
    ...
    let names = global.db.data.users[who].registered ? global.db.data.users[who].name : conn.getName(who)
    ...
    // const pp = global.db.data.users[m.sender].ppuser ? global.db.data.users[m.sender].ppuser : await this.profilePictureUrl(conn.user.jid).catch(_ => 'https://mycipongkor.com/wp-content/uploads/2022/05/Foto-profil-WA-kosong-senyum-min.jpg')
    //FAKE TROLI
