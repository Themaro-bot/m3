let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
    if (args.length < 3) {
        ...
        switch (type) {
            case 'miko': case 'miko':
                if (global.db.data.users[m.sender].miko >= count * 1) {
                    try {
                        global.db.data.users[m.sender].miko -= count * 1
                        global.db.data.users[who].miko += count * 1
                        conn.reply(m.chat, `Berhasil mentransfer MIKO sebesar ${count}`.trim(), m)
                    } catch (e) {
                        global.db.data.users[m.sender].miko += count * 1
                ...
                } else conn.reply(m.chat, `Uang kamu tidak mencukupi untuk mentransfer MIKO sebesar ${count}`.trim(), m)
                break
            case 'tabungan':
                if (global.db.data.users[m.sender].atm >= count * 1) {
                   try {
                       ...
                       conn.reply(m.chat, `Berhasil mentransfer Miko dari bank sebesar ${count}`.trim(), m)
                    } catch (e) {
                        global.db.data.users[m.sender].atm += count * 1
                        m.reply('Gagal Menstransfer')
                        console.log(e)
                ...
                } else conn.reply(m.chat, `Limit kamu tidak mencukupi untuk mentransfer Miko dari Bank sebesar ${count}`.trim(), m)
                break
            case 'limit':
            ...
            case 'iron':
                ...
                break
            default:
                return conn.reply(m.chat, `Gunakan format ${usedPrefix}transfer <type> <jumlah> <@tag>\n📍 Contoh penggunaan: *${usedPrefix}transfer miko 100 @tag*\n\n*List yang bisa di transfer*\n💹 miko\n💳 Tabungan\n🥤 Potion\n🗑️ Sampah\n💎 Diamond\n📦 Common\n🛍️ Uncommon\n🎁 Mythic\n🧰 Legendary\n🕸️ String\n🪵 Kayu\n🪨 Batu\n⛓️ Iron`.trim(), m)
        }
    } catch (e) {
        conn.reply(m.chat, `Gunakan format ${usedPrefix}transfer <type> <jumlah> <@tag>\📍 Contoh penggunaan: *${usedPrefix}transfer miko 100 @tag*\n\n*List yang bisa di transfer :*\n💹 miko\n💳 Tabungan\n🥤 Potion\n🗑️ Sampah\n💎 Diamond\n📦 Common\n🛍️ Uncommon\n🎁 Mythic\n🧰 Legendary\n🕸️ String\n🪵 Kayu\n🪨 Batu\n⛓ iron`.trim(), m)
        console.log(e)
        if (DevMode) {
            for (let jid of global.owner.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').filter(v => v != conn.user.jid)) {
                conn.reply(jid, 'Transfer.js error\nNo: *' + m.sender.split`@`[0] + '*\nCommand: *' + m.text + '*\n\n*' + e + '*', m)
