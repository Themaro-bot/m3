...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    if (user.lastdagang < cooldown) {
        if (5000 > user.miko) throw 'Target tidak memiliki modal harap masukkan modal 5000 Miko'
        if (5000 > user.miko) throw 'kamu tidak memiliki modal harap masukkan modal 5000 Miko'
        let caption = `${htki} BERDAGANG ${htka}\nMohon tunggu kak.. @${m.sender.split("@")[0]} dan @${who.split("@")[0]} sedang berdagang.. 😅`
        
        let _caption = `Selamat @${m.sender.split("@")[0]} dan @${who.split("@")[0]} mendapatkan MIKO..\n\nPenghasilan dagang @${m.sender.split("@")[0]} didapatkan +5000\n${user.miko += 5000} MIKO @${m.sender.split("@")[0]}\n\nPenghasilan dagang @${who.split("@")[0]} didapatkan +5000\n${user.miko += 5000} MIKO @${who.split("@")[0]}`
      
        if (user.miko < 10000) throw `Maaf kamu tidak bisa berdagang dengan ${who.split("@")[0]} karena jumlah modal yang kamu keluarkan kurang dari 10000 MIKO`
        
        let __caption = `${htki} SUKSES ${htka}\nSelamat @${m.sender.split("@")[0]} dan @${who.split("@")[0]} mendapatkan MIKO..\n\nPenghasilan dagang @${m.sender.split("@")[0]} didapatkan +10000\n${user.miko += 10000} MIKO @${m.sender.split("@")[0]}\n\nPenghasilan dagang @${who.split("@")[0]} didapatkan +10000\n${user.miko += 10000} MIKO @${who.split("@")[0]}`
