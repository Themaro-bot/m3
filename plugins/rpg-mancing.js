...
let handler = async (m, { conn, command, args, usedPrefix }) => {
     ...
     let pemancing = global.db.data.users[m.sender].registered ? global.db.data.users[m.sender].name : conn.getName(m.sender)
          ...
          if (/fishing|mancing/i.test(command)) {
               switch (type) {
                    ...
                    case 'emosi':
                         ...
                         if (new Date - pengguna.lastfishing < 10800000) throw m.reply(`Kamu masih kelelahan\nHarap tunggu *${timers}* lagi`)
                         ...
                         if (pengguna.fishingrod > 0) {
                              if (new Date - pengguna.lastfishing > 240000) {
                                   ...
                                   let paus = `${Math.floor(Math.random() * 0)}`.trim()
...
${usedPrefix}cook`
                         ...
                         // conn.reply(m.chat,`*[❗] kamu tidak punya kail pancingan dan umpan 100 🎣*\n\nketik .buy pancingan 1 untuk buy\n\nketik .buy umpan 100 untuk buy`, m)
                         break
