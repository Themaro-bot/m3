const items = [
    'miko', 'potion', 'trash', 'wood',
    'batu', 'string', 'petFood', 'emerald',
...
example ${usedPrefix}${command} miko 9999 @user
...
`.trim()
    ...
    user[type] -= count * 1
    _user[type] += count * 1
    if (previous > user[type] * 1 && _previous < _user[type] * 1) m.reply(`Succes transfer *${count}* ${rpg.emoticon(type)}${type}${special(type)} to *@${(who || '').replace(/@s\.whatsapp\.net/g, '')}*`, null, { mentions: [who] })
    else {
        user[type] = previous
        _user[type] = _previous
        m.reply(`Failted to transfer *${count}* ${rpg.emoticon(type)}${type}${special(type)} to *@${(who || '').replace(/@s\.whatsapp\.net/g, '')}*`, null, { mentions: [who] })
    }
    //     let confirm = `
