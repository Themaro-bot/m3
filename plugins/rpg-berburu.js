let handler = async (m, {
	...
	if (new Date - global.db.data.users[m.sender].lastberbru > 500000 || global.db.data.users[m.sender].lastberbru == 0) {
...
*${author}*`
		...
		setTimeout(() => {
			conn.reply(m.chat,  `${name} ......`, m)
		}, 15000)

		setTimeout(() => {
			conn.reply(m.chat,  `${name} Dapet nih..`, m)
		}, 14000)

		setTimeout(() => {
			conn.reply(m.chat,  `${name} Sedang berburu...`, m)
		}, 0)
