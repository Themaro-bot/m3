...
export async function handler(chatUpdate) {
    ...
    global.img = 'hhttps://i.ibb.co/zbhmXsn/bg.jpg' 
    ...
    try {
        ...
        try {
            ...
            if (!("BannedReason" in user)) user.BannedReason = ""
              ...
              if (!("putuspasangan" in user)) user.putuspasangan = ""
              ...
              if (!isNumber(user.mangga)) user.mangga = 0
              if (!isNumber(user.miko)) user.miko = 0
        ...
        case 'remove':
            if (chat.welcome) {
                ...
                let welc = 'WELCOME'
                    ...
                    try {
                        pp = global.db.data.users[m.sender].ppuser ? global.db.data.users[m.sender].ppuser : await this.profilePictureUrl(m.sender, 'image').catch(_ => 'https://mycipongkor.com/wp-content/uploads/2022/05/Foto-profil-WA-kosong-senyum-min.jpg')
                    } catch (e) {
                    } finally {
...
let msg = {
    ...
    nsfw: `🚩 NSFW Feature only for premium/The group admin disable nsfw on this group.`,
